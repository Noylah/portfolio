...
export default function Card({ title, description, image, primaryBtn, secondaryBtn, primaryBtnClick, secondaryBtnClick, color = "emerald"}: CardProps) {
    ...
    return (
        <div className="shrink-0 w-full max-w-[384px] p-4 bg-slate-800 rounded-2xl border border-slate-700 flex flex-col">
            <div className={`h-44 w-full rounded-xl flex items-center justify-center mb-2 ${currentBg}`}>
                    {image ? <img src={image} alt="Card Image" className="w-full h-full object-cover rounded-xl"></img> : ""}
            </div>
            <div className="p-2 flex flex-col mb-4">
                <h1 className="text-white font-semibold text-2xl">{title}</h1>
                <p className="text-slate-400 text-sm">{description}</p>
            </div>
            <div className="self-end mt-auto py-2">
